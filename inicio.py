...
def percepciones():
    ...
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_percepciones.html", percepciones=datos)
...
def agr_percepcion():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_periodo, Descripcion from periodo')
    datos1 = cursor.fetchall()
    cursor.execute('select idtipo_nomina, descripcion from tipo_nomina')
    datos2 = cursor.fetchall()
    conn.close()
    return render_template("agrega_percepciones.html", periodos=datos1, tipos=datos2)

@app.route('/agrega_percepciones', methods=['POST'])
def agrega_percepciones():
    if request.method == 'POST':
        aux_conc = request.form['concepto']
        aux_per = request.form['periodo']
        aux_mes = request.form['mes']
        aux_dia = request.form['dia']
        aux_t_con = request.form['tipo_concepto']
        aux_t_nom = request.form['tipo']
        aux_valor = request.form['valor']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from percepcion where Descripcion = %s',(aux_conc))
        perc = cursor.fetchone
        #if (perc[0] != 0):
         #   error = "La percepcion ya se encuentra agregada."
          #  return render_template("error.html", des_error=error, paginaant="/percepciones")
        #else:
        cursor.execute('insert into percepcion (Descripcion, id_periodo, Dia, Mes, Nom_per, Tipo_nom, Dia_per) values (%s,%s,%s,%s,%s,%s,%s)',(aux_conc, aux_per,aux_dia, aux_mes, aux_t_con, aux_t_nom,aux_valor))
        conn.commit()
        conn.close()
